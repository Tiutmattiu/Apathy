...
function renderChoice(pg,a){const g=el('div','direct');pg.options.forEach(o=>{const b=btn(o[1],()=>{set(pg.key,o[0]);if(pg.key==='education_level'&&o[2]!==null&&!present('education_years'))set('education_years',o[2]);if(pg.key==='pd_status_self_report')set('pd_hc_status',Number(o[0])===1?'PD':'HC');b.classList.add('selected');setTimeout(autoNext,120)},'choice'+(sameValue(val(pg.key),o[0])?' selected':''));g.append(b)});a.append(g);if(pg.key==='education_level'&&val(pg.key)==='other'){const f=fieldText('實際教育年數','education_years','請輸入年數','number');a.append(f)}}
...
function calcAge(){const s=val('date_of_birth');if(s){const b=new Date(s+'T00:00:00'),n=new Date();let y=n.getFullYear()-b.getFullYear();if(n.getMonth()<b.getMonth()||(n.getMonth()===b.getMonth()&&n.getDate()<b.getDate()))y--;if(Number.isFinite(y)&&y>=0&&y<=120)return y}const manual=Number(val('age_years'));return Number.isFinite(manual)&&manual>=0&&manual<=120?manual:null}
...
function backfill(){const m=el('main','backfill');ROOT.innerHTML='';ROOT.append(el('div','topline'),m);m.append(toolbar('歷史資料補錄'));m.append(identityStrip());const nav=el('nav','backfill-nav');const sections=[];function sec(id,title){const s=el('section','bf-section');s.id='bf-'+id;s.append(el('h2','',title));m.append(s);sections.push([id,title]);return s}
 const id=sec('identity','Participant身份及基本資料');id.append(fieldIdDigits('P_ID','p_id',val('participant_series')==='Y'?'Y':'P','例：140'),fieldIdDigits('S_ID（如紙本有）','s_id','S','例：082'),fieldText('姓名','participant_name',''),fieldText('香港身份證頭四個字','hkid_prefix4',''));addBFDate(id,'出生日期','date_of_birth');addBFChoices(id,'性別','gender',[['M','M'],['F','F']]);addBFChoices(id,'PD／HC身份','pd_hc_status',C.identities);id.append(fieldText('聯絡電話','contact_phone',''),fieldText('評估時實際年齡','age_years','歲','number'),fieldText('實際教育年數','education_years','年','number'));
...
const APP_BUILD='10.0.3-contract-minimal';
...
function screeningPages(){
  ...
  p.push(inputPage('身分及基本資料','香港身份證頭四個字','hkid_prefix4','例：A123'));
  p.push({section:'身分及基本資料',kind:'dob',label:'出生日期'});
  p.push(inputPage('身分及基本資料','評估時實際年齡','age_years','請輸入整數年齡'));
  p.push(choicePage('身分及基本資料','性別','gender',[['M','M'],['F','F']]));
  p.push(choicePage('身分及基本資料','已確認研究身份','pd_hc_status',C.identities));
  p.push(inputPage('身分及基本資料','聯絡電話','contact_phone','例：9123 4567'));
  p.push(choicePage('身分及基本資料','招募來源','recruitment_source_code',C.recruitment));
  p.push(choicePage('身分及基本資料','教育程度','education_level',C.education.map(x=>[x[0],x[1],x[2]])));
  p.push(inputPage('身分及基本資料','實際受教育年數','education_years','請輸入實際年數'));
  p.push({section:'身分及基本資料',kind:'positiveOne',label:'Participant自述PD狀態',key:'pd_status_self_report',button:'有PD',conditional:{key:'pd_duration_years_self_report',label:'自述PD病程',unit:'年'}});
  p.push({section:'MoCA',kind:'moca',label:'MoCA原始總分'});
...
function payload(form,event,status){
  ...
  if(clean.age_years!==null&&clean.age_years!==undefined&&clean.age_years!=='')clean.age_source=clean.date_of_birth?'staff_recorded_with_dob':'staff_recorded';
...
/* Backfill-only: focus never leaves the PDI matrix for IOR. */
