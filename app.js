...
function renderRBMain(a){const f=el('div','field');f.append(el('div','label','資料由誰提供？'));const src=el('div','direct');B.rbdsq.sourceOptions.forEach(o=>src.append(btn(o.label,()=>{set(B.rbdsq.sourceField,o.value);player()},sameValue(val(B.rbdsq.sourceField),o.value)?'selected':'')));f.append(src);a.append(f);const g=el('div','toggle-grid');B.rbdsq.items.forEach(i=>g.append(toggleButton(i.fullLabel,i.name,()=>player())));a.append(g,el('p','hint','未選項目代表「否」。'))}
...
function addBFChoices(parent,label,key,options){const w=el('div','field');w.append(el('div','label',label));const g=el('div','direct');options.forEach(o=>g.append(btn(o[1],()=>{set(key,o[0]);backfill()},sameValue(val(key),o[0])?'selected':'')));w.append(g);parent.append(w)}
...
function addStaffChoices(s,label,key,opts){const f=el('div','field');f.append(el('div','label',label));const g=el('div','direct');opts.forEach(o=>g.append(btn(o[1],()=>{set(key,o[0]);renderByFlow()},sameValue(val(key),o[0])?'selected':'')));f.append(g);s.append(f)}
...
const APP_BUILD='10.0.1-field-fixes';
...
function renderMRIAdminFields(a){
  ...
  if(val('electronic_payment_available')===1){const p=el('div','direct');[['payme','PayMe'],['alipayhk','AlipayHK'],['fps','轉數快（FPS）']].forEach(x=>p.append(btn(x[1],()=>{set('electronic_payment_method',x[0]);player()},sameValue(val('electronic_payment_method'),x[0])?'selected':'')));w.append(el('h4','','電子收款方式'),p);if(present('electronic_payment_method'))w.append(fieldText('收款電話','electronic_payment_phone','例：9123 4567'))}
  a.append(w)
}
...
function addBFChoices(parent,label,key,options){
  ...
  options.forEach(o=>{const b=btn(o[1],e=>{e?.stopPropagation?.();set(key,o[0]);requestAnimationFrame(backfill)},sameValue(val(key),o[0])?'selected':'');b.dataset.field=key;b.dataset.value=String(o[0]);g.append(b)});
...
function addStaffChoices(s,label,key,opts){
  ...
  opts.forEach(o=>{const b=btn(o[1],e=>{e?.stopPropagation?.();set(key,o[0]);b.classList.add('selected');requestAnimationFrame(renderByFlow)},sameValue(val(key),o[0])?'selected':'');b.dataset.field=key;b.dataset.value=String(o[0]);g.append(b)});
