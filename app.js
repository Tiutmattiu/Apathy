...
function backfill(){const m=el('main','backfill');ROOT.innerHTML='';ROOT.append(el('div','topline'),m);m.append(toolbar('歷史資料補錄'));m.append(identityStrip());const nav=el('nav','backfill-nav');const sections=[];function sec(id,title){const s=el('section','bf-section');s.id='bf-'+id;s.append(el('h2','',title));m.append(s);sections.push([id,title]);return s}
 const id=sec('identity','Participant身份及基本資料');id.append(fieldIdDigits('P_ID','p_id',val('participant_series')==='Y'?'Y':'P','例：140'),fieldIdDigits('S_ID（如紙本有）','s_id','S','例：082'),fieldText('姓名','participant_name',''),fieldText('香港身份證頭四個字','hkid_prefix4',''));addBFDate(id,'出生日期','date_of_birth');addBFChoices(id,'性別','gender',[['M','M'],['F','F']]);addBFChoices(id,'PD／HC身份','pd_hc_status',C.identities);id.append(fieldText('聯絡電話','contact_phone',''),fieldText('實際教育年數','education_years','年','number'));
...
/* ===== v10.0.3 FINAL field fixes; preserve the site's existing selected-blue CSS ===== */
const FINAL_FIX_BUILD='10.0.3-final';

function selectWithExistingBlue(button,group){
  group?.querySelectorAll('button.selected').forEach(x=>x.classList.remove('selected'));
...
document.addEventListener('click',e=>{
  ...
  if(b.classList.contains('choice')||b.closest('.scale-buttons,.options'))selectWithExistingBlue(b,group);
...
function renderPositiveOne(pg,a){
  ...
  g.append(
    btn(pg.button,()=>{set(pg.key,1);player()},'choice'+(sameValue(val(pg.key),1)?' selected':'')),
    btn('沒有PD',()=>{set(pg.key,0);if(pg.conditional)set(pg.conditional.key,null);setTimeout(autoNext,220)},'choice'+(sameValue(val(pg.key),0)?' selected':''))
  );a.append(g);
  if(sameValue(val(pg.key),1)&&pg.conditional){
    ...
    const save=()=>{const n=i.value===''?null:Number(i.value);if(n===null||!Number.isFinite(n)||n<0||n>80)return false;set(pg.conditional.key,n);return true};
    i.oninput=()=>{clearTimeout(finalAdvanceTimer);if(save())finalAdvanceTimer=setTimeout(()=>{if(playerPages()[ST.step]?.kind==='positiveOne')autoNext()},650)};
    i.onkeydown=e=>{if(e.key==='Enter'&&save()){e.preventDefault();clearTimeout(finalAdvanceTimer);autoNext()}};
    i.onblur=()=>{if(save()&&playerPages()[ST.step]?.kind==='positiveOne'){clearTimeout(finalAdvanceTimer);setTimeout(autoNext,100)}};
    f.append(i,document.createTextNode(' '+pg.conditional.unit));a.append(f)
  }
}

let finalMocaTimer=null;
function renderMoca(a){
  const i=el('input','text');i.type='number';i.min=0;i.max=30;i.inputMode='numeric';i.placeholder='0–30';i.value=val('moca_1_raw_total')??'';const result=el('div');
  const valid=()=>i.value!==''&&Number(i.value)>=0&&Number(i.value)<=30;
  const update=()=>{const raw=i.value===''?null:Number(i.value);set('moca_1_raw_total',raw);result.innerHTML='';const age=calcAge(),er=val('education_years'),edu=er===null||er===''?null:Number(er);if(raw===null||age===null||edu===null||!Number.isFinite(edu)){result.append(el('div','result','輸入總分並完成出生日期與教育資料後，系統會計算結果。'));return}const adj=edu<=12?1:0,adjusted=Math.min(30,raw+adj),cut=mocaCutoff(age,edu);Object.assign(ST.answers,{moca_1_adjustment:adj,moca_1_adjusted_total:adjusted,moca_1_age_years:age,moca_1_education_years:edu,moca_1_16th_cutoff:cut,moca_1_norm_result_code:cut===null?null:(raw>cut?1:0)});saveDraft();result.append(resultBox('MoCA結果',[`原始總分：${raw}/30`,`教育調整：+${adj}`,`調整後總分：${adjusted}/30`,`第16百分位Cutoff：${cut===null?'需覆核':cut}`],cut!==null&&raw<=cut?'bad':'good'))};
  i.oninput=()=>{clearTimeout(finalMocaTimer);if(!valid()){i.setCustomValidity('MoCA原始總分必須為0至30');return}i.setCustomValidity('');update();finalMocaTimer=setTimeout(()=>{if(playerPages()[ST.step]?.kind==='moca')autoNext()},800)};
  i.onkeydown=e=>{if(e.key==='Enter'&&valid()){e.preventDefault();clearTimeout(finalMocaTimer);update();autoNext()}};
  i.onblur=()=>{if(valid()&&playerPages()[ST.step]?.kind==='moca'){clearTimeout(finalMocaTimer);setTimeout(autoNext,100)}};a.append(i,result);update()
}

let finalIorTimer=null;
function finalIorSet(pg,dim,value){
  const n=String(pg.scenario).padStart(2,'0');set(`ior${n}_${dim}`,value);clearTimeout(finalIorTimer);player();
  if(['frequency','conviction','distress'].every(k=>present(`ior${n}_${k}`))){const step=ST.step;finalIorTimer=setTimeout(()=>{if(ST.step===step&&playerPages()[ST.step]?.kind==='iorScenario')autoNext()},700)}
}
function renderIORScenario(pg,a){
  const n=String(pg.scenario).padStart(2,'0');a.append(el('p','context',pg.scenarioText));
  ...
  dims.forEach(z=>{const block=el('div','ior-block');block.append(el('h4','',z[1]));const g=el('div','scale-buttons');z[2].forEach((lab,i)=>{const value=i+1,key=`ior${n}_${z[0]}`;g.append(btn(`${value} ${lab}`,()=>finalIorSet(pg,z[0],value),val(key)===value?'selected':''))});block.append(g);a.append(block)})
}
document.addEventListener('keydown',e=>{
  ...
  const pg=playerPages()[ST.step];if(pg?.kind!=='iorScenario')return;e.preventDefault();const n=String(pg.scenario).padStart(2,'0'),dims=['frequency','conviction','distress'];let ix=dims.findIndex(k=>!present(`ior${n}_${k}`));if(ix<0)ix=2;finalIorSet(pg,dims[ix],Number(e.key))
});

/* ===== FINAL authoritative Medication / LEDD layer =====
   Machine route: paste the original medication list and parse it.
   Manual route: choose only from the built-in catalogue.
   Both routes, differences and audit JSON are saved. */
const MED_CATALOGUE=[
  {id:'levodopa_ir',label:'Levodopa IR',aliases:['levodopa','l-dopa'],factor:1,category:'levodopa',component:'single'},
  {id:'sinemet',label:'Sinemet／Carbidopa-Levodopa',aliases:['sinemet','carbidopa levodopa','carbidopa/levodopa','息寧'],factor:1,category:'levodopa',component:'second'},
  {id:'sinemet_cr',label:'Sinemet CR／Levodopa CR',aliases:['sinemet cr','sinemet controlled release','levodopa cr','息寧控釋'],factor:.75,category:'levodopa',component:'second'},
  {id:'madopar',label:'Madopar／美多芭（Co-beneldopa）',aliases:['madopar','medopar','madopar','美多芭','co-beneldopa'],factor:1,category:'levodopa',component:'first'},
  {id:'stalevo',label:'Stalevo／達靈復',aliases:['stalevo','levodopa entacapone','達靈復'],factor:1.33,category:'levodopa',component:'first'},
  {id:'duodopa',label:'Duodopa',aliases:['duodopa'],factor:1.11,category:'levodopa',component:'single'},
  {id:'rytary',label:'Rytary',aliases:['rytary'],factor:.6,category:'levodopa',component:'single'},
  {id:'pramipexole',label:'Pramipexole／Mirapex／Sifrol／森福羅',aliases:['pramipexole','mirapex','sifrol','森福羅'],factor:100,category:'da',component:'single'},
  {id:'ropinirole',label:'Ropinirole／Requip',aliases:['ropinirole','ropinrole','requip'],factor:20,category:'da',component:'single'},
  {id:'rotigotine',label:'Rotigotine／Neupro／優普洛',aliases:['rotigotine','neupro','優普洛'],factor:30,category:'da',component:'single'},
  {id:'apomorphine',label:'Apomorphine',aliases:['apomorphine'],factor:10,category:'da',component:'single'},
  {id:'bromocriptine',label:'Bromocriptine',aliases:['bromocriptine'],factor:10,category:'da',component:'single'},
  {id:'cabergoline',label:'Cabergoline',aliases:['cabergoline'],factor:80,category:'da',component:'single'},
  {id:'rasagiline',label:'Rasagiline／Azilect／雷沙吉蘭',aliases:['rasagiline','rasagline','azilect','雷沙吉蘭'],factor:100,category:'other',component:'single'},
  {id:'selegiline',label:'Selegiline oral／司來吉蘭',aliases:['selegiline','司來吉蘭'],factor:10,category:'other',component:'single'},
  {id:'amantadine',label:'Amantadine／金剛烷胺',aliases:['amantadine','金剛烷胺'],factor:1,category:'other',component:'single'},
  {id:'entacapone_only',label:'Entacapone（獨立；需配套Levodopa覆核）',aliases:['entacapone','恩他卡朋'],factor:null,category:'review',component:'single'},
  {id:'tolcapone_only',label:'Tolcapone（獨立；需配套Levodopa覆核）',aliases:['tolcapone'],factor:null,category:'review',component:'single'},
  {id:'other',label:'其他／未識別（不自動計算）',aliases:[],factor:null,category:'review',component:'single'}
];
function medById(id){return MED_CATALOGUE.find(x=>x.id===id)||MED_CATALOGUE[MED_CATALOGUE.length-1]}
function medRound(n){return Number.isFinite(Number(n))?Number(Number(n).toFixed(2)):null}
function medLines(raw){return String(raw||'').replace(/\\n/g,'\n').replace(/[；;]/g,'\n').split(/\n+/).map(x=>x.trim()).filter(Boolean)}
function medFind(line){const x=String(line).toLowerCase().replace(/[-_]+/g,' ');return MED_CATALOGUE.find(d=>d.aliases.some(a=>x.includes(a.toLowerCase())))||medById('other')}
function medFrequency(line,warnings){const x=String(line).toLowerCase();let m=x.match(/(\d+(?:\.\d+)?)\s*[-–]\s*(\d+(?:\.\d+)?)\s*(?:times?|次)?\s*(?:\/|per\s*)?(?:day|daily|日|天)?/);if(m){warnings.push(`頻次範圍採上限 ${m[2]}/day`);return +m[2]}m=x.match(/(?:每日|每天|daily|per\s*day|\/day)\s*(\d+(?:\.\d+)?)/);if(m)return +m[1];m=x.match(/(\d+(?:\.\d+)?)\s*(?:times?|次)\s*(?:\/|per\s*)?(?:day|daily|日|天)/);if(m)return +m[1];if(/\b(?:bd|bid)\b|twice daily/.test(x))return 2;if(/\b(?:tds|tid)\b|three times daily/.test(x))return 3;if(/\b(?:qds|qid)\b|four times daily/.test(x))return 4;if(/\b(?:od)\b|once daily/.test(x))return 1;return null}
function medUnits(line){const x=String(line).toLowerCase();let m=x.match(/(?:每次|each)\s*(\d+(?:\.\d+)?)/);if(m)return +m[1];m=x.match(/(\d+(?:\.\d+)?)\s*(?:tab(?:let)?s?|caps?(?:ules?)?|pill?s?|粒|片)(?:\s*\/\s*(?:dose|次))?/);if(m)return +m[1];if(/(?:half|半)\s*(?:tab(?:let)?|pill|片|粒)?/.test(x))return .5;return 1}
function medDose(line,drug,warnings){let slash=String(line).match(/(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)(?:\s*\/\s*(\d+(?:\.\d+)?))?/);if(!slash&&['sinemet','sinemet_cr'].includes(drug.id)&&!/\d+\s*mg/i.test(line)){warnings.push('未寫Sinemet strength；不能安全推定規格');return null}if(slash){const a=[+slash[1],+slash[2],slash[3]?+slash[3]:null];return drug.component==='second'?a[1]:a[0]}const mg=String(line).match(/(\d+(?:\.\d+)?)\s*mg/i);if(!mg)return null;const n=+mg[1];if(drug.id==='madopar'&&n===125){warnings.push('Madopar 125mg按Levodopa 100mg');return 100}if(drug.id==='madopar'&&n===250){warnings.push('Madopar 250mg按Levodopa 200mg');return 200}return n}
function parseMedicationRaw(raw){const rows=[],warnings=[];const totals={levodopa:0,da:0,other:0};medLines(raw).forEach((line,i)=>{const local=[],drug=medFind(line);if(drug.id==='other'){rows.push({line:i+1,raw:line,status:'unresolved',reason:'未識別藥物'});warnings.push(`第${i+1}行未識別藥物`);return}if(/\b(?:prn|as needed|upon need)\b/i.test(line)&&medFrequency(line,[])===null){rows.push({line:i+1,raw:line,drug:drug.label,status:'ignored',reason:'PRN且無固定頻次',ledd:0});return}if(drug.factor===null){rows.push({line:i+1,raw:line,drug:drug.label,status:'unresolved',reason:'需要配套Levodopa資料及人工覆核'});warnings.push(`第${i+1}行${drug.label}無法獨立換算`);return}const dose=medDose(line,drug,local),times=medFrequency(line,local),units=medUnits(line);if(dose===null||times===null){rows.push({line:i+1,raw:line,drug:drug.label,status:'unresolved',dose,times,units,reason:dose===null?'缺少可識別mg規格':'缺少固定每日頻次',warnings:local});warnings.push(`第${i+1}行資料不足`);return}const daily=medRound(dose*times*units),ledd=medRound(daily*drug.factor);totals[drug.category]+=ledd;rows.push({line:i+1,raw:line,medicationId:drug.id,drug:drug.label,category:drug.category,doseMg:dose,timesPerDay:times,unitsPerTime:units,dailyMg:daily,factor:drug.factor,ledd,status:'ok',warnings:local});local.forEach(x=>warnings.push(`第${i+1}行：${x}`))});const total=medRound(totals.levodopa+totals.da+totals.other)||0;return{rows,totals:{levodopa:medRound(totals.levodopa)||0,da:medRound(totals.da)||0,other:medRound(totals.other)||0},total,warnings,unresolved:rows.filter(x=>x.status==='unresolved').length,complete:rows.some(x=>x.status==='ok')&&!rows.some(x=>x.status==='unresolved')}}
function normalizeManualMedication(){ST.meds=ST.meds.map(m=>m.medicationId!==undefined?m:{medicationId:'',strength:m.strength||'',times:m.times||'',units:m.units||''})}
function calculateManualMedication(){normalizeManualMedication();let unresolved=0;const totals={levodopa:0,da:0,other:0};const rows=ST.meds.map((m,index)=>{const drug=medById(m.medicationId),dose=Number(m.strength),times=Number(m.times),units=Number(m.units);if(!m.medicationId&&!m.strength&&!m.times&&!m.units)return{index:index+1,status:'empty'};if(!m.medicationId||drug.factor===null||![dose,times,units].every(Number.isFinite)||dose<=0||times<=0||units<=0){unresolved++;return{index:index+1,status:'unresolved',drug:drug.label}}const daily=medRound(dose*times*units),ledd=medRound(daily*drug.factor);totals[drug.category]+=ledd;return{index:index+1,medicationId:drug.id,drug:drug.label,doseMg:dose,timesPerDay:times,unitsPerTime:units,dailyMg:daily,factor:drug.factor,category:drug.category,ledd,status:'ok'}});const total=medRound(totals.levodopa+totals.da+totals.other)||0;return{rows,totals:{levodopa:medRound(totals.levodopa)||0,da:medRound(totals.da)||0,other:medRound(totals.other)||0},total,unresolved,complete:rows.some(x=>x.status==='ok')&&unresolved===0}}
function saveMedicationAudit(machine,manual){[['machine',machine],['manual',manual]].forEach(([p,x])=>{setDerived(`ledd_${p}_levodopa`,x.complete?x.totals.levodopa:null);setDerived(`ledd_${p}_da`,x.complete?x.totals.da:null);setDerived(`ledd_${p}_other`,x.complete?x.totals.other:null);setDerived(`ledd_${p}_total`,x.complete?x.total:null);setDerived(`ledd_${p}_status`,x.complete?'complete':x.rows.some(r=>r.status!=='empty')?'partial':'empty')});['levodopa','da','other'].forEach(k=>setDerived(`ledd_difference_${k}`,machine.complete&&manual.complete?medRound(manual.totals[k]-machine.totals[k]):null));setDerived('ledd_difference_total',machine.complete&&manual.complete?medRound(manual.total-machine.total):null);setDerived('ledd_match_flag',machine.complete&&manual.complete?(Math.abs(manual.total-machine.total)<.01?1:0):null);setDerived('medication_machine_parse_json',JSON.stringify(machine));setDerived('medication_manual_review_json',JSON.stringify(manual));setDerived('medication_raw_text',val('medication_machine_raw_text')||'')}
function medNumber(value,placeholder,onchange){const i=el('input','text');i.type='number';i.min='0';i.step='0.01';i.inputMode='decimal';i.placeholder=placeholder;i.value=value??'';i.oninput=()=>onchange(i.value);return i}
function renderMedicationRows(s){normalizeManualMedication();const machineBlock=el('div','plain-block');machineBlock.append(el('h4','','A. 原始藥單機器解析'),el('p','hint','直接貼上手寫或醫院藥單原文；建議每種藥物一行。系統會模糊解析，所有結果仍須由工作人員核對。'));const raw=el('textarea');raw.placeholder='例：Madopar 125 mg 1 tab 3 times/day';raw.value=val('medication_machine_raw_text')||'';raw.oninput=()=>set('medication_machine_raw_text',raw.value);machineBlock.append(raw,btn('解析／重新解析藥單',()=>{set('medication_machine_raw_text',raw.value);renderByFlow()},'primary'));s.append(machineBlock);const machine=parseMedicationRaw(val('medication_machine_raw_text')||'');if(machine.rows.length){const p=el('div','plain-block');p.append(el('strong','','機器解析明細'));machine.rows.forEach(r=>p.append(el('div','med-row',`${r.line}. ${r.drug||r.raw}｜規格 ${r.doseMg??'待核實'} mg｜每日 ${r.timesPerDay??r.times??'待核實'} 次｜每次 ${r.unitsPerTime??r.units??'待核實'}｜LEDD ${r.ledd??'—'} mg${r.reason?'｜'+r.reason:''}`)));s.append(p)}const manualBlock=el('div','plain-block');manualBlock.append(el('h4','','B. 人工核對（內建藥物清單）'),el('p','hint','藥名只可從內建清單選擇，不提供自由輸入。'));ST.meds.forEach((m,n)=>{const r=el('div','med-row'),sel=el('select');sel.append(new Option('— 選擇藥物／製劑 —',''));MED_CATALOGUE.forEach(x=>sel.append(new Option(`${x.label}${x.factor===null?'':'（×'+x.factor+'）'}`,x.id)));sel.value=m.medicationId||'';sel.onchange=()=>{m.medicationId=sel.value;saveDraft();renderByFlow()};r.append(sel,medNumber(m.strength,'有效成分mg／每單位',v=>{m.strength=v;saveDraft()}),medNumber(m.times,'每日次數',v=>{m.times=v;saveDraft()}),medNumber(m.units,'每次片／單位',v=>{m.units=v;saveDraft()}),btn('刪除',()=>{ST.meds.splice(n,1);saveDraft();renderByFlow()},'linkbtn'));manualBlock.append(r)});manualBlock.append(btn('＋新增人工核對藥物',()=>{ST.meds.push({medicationId:'',strength:'',times:'',units:''});saveDraft();renderByFlow()},'primary'));s.append(manualBlock)}
function renderLeddPanel(s){const machine=parseMedicationRaw(val('medication_machine_raw_text')||''),manual=calculateManualMedication();saveMedicationAudit(machine,manual);safeSave();s.append(resultBox('機器LEDD（三項）',machine.complete?[`Levodopa：${machine.totals.levodopa} mg｜DA：${machine.totals.da} mg｜其他：${machine.totals.other} mg`,`Total：${machine.total} mg`,`Warnings：${machine.warnings.join('；')||'沒有'}`]:[`正式結果暫不可計算`,`解析：${machine.rows.length}行；Unresolved：${machine.unresolved}`,`Warnings：${machine.warnings.join('；')||'尚未貼上完整藥單'}`],machine.complete?'good':'warn'));s.append(resultBox('人工LEDD（三項）',manual.complete?[`Levodopa：${manual.totals.levodopa} mg｜DA：${manual.totals.da} mg｜其他：${manual.totals.other} mg`,`Total：${manual.total} mg`]:[`正式結果暫不可計算`,`未完整／未識別：${manual.unresolved||'尚未加入人工核對藥物'}`],manual.complete?'good':'warn'));const d=machine.complete&&manual.complete?{l:medRound(manual.totals.levodopa-machine.totals.levodopa),a:medRound(manual.totals.da-machine.totals.da),o:medRound(manual.totals.other-machine.totals.other),t:medRound(manual.total-machine.total)}:null;s.append(resultBox('雙路徑差異',[`Levodopa差異（人工－機器）：${d?d.l+' mg':'—'}`,`DA差異（人工－機器）：${d?d.a+' mg':'—'}`,`其他差異（人工－機器）：${d?d.o+' mg':'—'}`,`Total差異（人工－機器）：${d?d.t+' mg':'—'}`,`核對：${!d?'待兩套完整':Math.abs(d.t)<.01?'一致':'需要覆核'}`],d&&Math.abs(d.t)>=.01?'warn':d?'good':''))}
function renderMedicationBF(s){renderMedicationRows(s);renderLeddPanel(s)}
const _validateClinicalBeforeFinalMedication=validateClinical;
validateClinical=function(s){const machine=parseMedicationRaw(val('medication_machine_raw_text')||''),manual=calculateManualMedication();saveMedicationAudit(machine,manual);const missing=[];if(!val('updrs3_route'))missing.push('UPDRS Part III路徑');if(val('updrs3_route')==='hospital_total_only'&&val('updrs3_reported_total')===null)missing.push('UPDRS總分');if(['hospital_items','research_assessed'].includes(val('updrs3_route'))&&updrsTotal().count<33)missing.push('UPDRS 33項');if(!machine.complete)missing.push('機器藥單解析仍有未解決項目');if(!manual.complete)missing.push('人工藥物核對未完整');if(missing.length){showInlineError(s,'尚未完成：'+missing.join('、'));return}const t=updrsTotal();if(t.count===33){set('updrs3_calculated_total',t.total);set('updrs3_total',t.total)}setDerived('total_ledd_mg',manual.total);setDerived('levodopa_ledd_mg',manual.totals.levodopa);setDerived('da_ledd_mg',manual.totals.da);safeSave();submitPayload('clinical','clinical_supplement','submitted')};

/* Use the site's original primary blue for every selected control.
   This intentionally removes the later, darker selected-blue override. */
(function installOriginalBlue(){const style=document.createElement('style');style.id='apathy-selected-blue-unifier';style.textContent=`
:root{--apathy-selected-blue:#1976b9}
button.selected,.choice.selected,.toggle.selected,.scale-buttons button.selected,.options button.selected,button[data-field].selected,button[data-updrs-key].selected{background:var(--apathy-selected-blue)!important;background-color:var(--apathy-selected-blue)!important;border-color:var(--apathy-selected-blue)!important;color:#fff!important}
button.selected *,.choice.selected *,.toggle.selected *,.scale-buttons button.selected *,.options button.selected *{color:#fff!important}
`;document.head.append(style);requestAnimationFrame(()=>{const primary=document.querySelector('button.primary');if(primary){const c=getComputedStyle(primary).backgroundColor;if(c&&c!=='rgba(0, 0, 0, 0)'&&c!=='transparent')document.documentElement.style.setProperty('--apathy-selected-blue',c)}})})();

/* MRI Sequence remark remains visible and editable at all times. */
document.addEventListener('click',e=>{if(ST.flow!=='mri_visit'||!e.target.closest('.chips button.toggle.danger'))return;setTimeout(()=>{const t=qa('textarea').find(x=>x.placeholder.includes('Sequence'));if(t){t.scrollIntoView({behavior:'smooth',block:'center'});t.focus()}},80)},true);
window.APATHY_APP_BUILD=FINAL_FIX_BUILD;
