...
/* ===== Medication rebuild M1 from validated Checkpoint D only ===== */
const MED_CATALOG_M1=Object.freeze([
 {id:'pregabalin',label:'Pregabalin',aliases:['pregabalin'],cat:'non_ledd',factor:null,component:'single'},
 {id:'rotigotine',label:'Rotigotine / Neupro',aliases:['rotigotine','rotigptine','rotigtptine','neupro'],cat:'da',factor:30,component:'single'},
 {id:'safinamide',label:'Safinamide / Xadago',aliases:['safinamide','mysylate','myslate','xadago'],cat:'review',factor:null,component:'single'},
 {id:'senna',label:'Senna',aliases:['senna','sennoside'],cat:'non_ledd',factor:null,component:'single'},
 {id:'sinemet_cr',label:'Sinemet CR / Carbidopa-Levodopa CR',aliases:['sinemet cr','sinemet 50/200 cr','carbidopa levodopa cr'],cat:'levodopa',factor:0.75,component:'second'},
 {id:'sinemet',label:'Sinemet / Carbidopa-Levodopa',aliases:['sinemet','carbidopa levodopa','carbidopa/levodopa'],cat:'levodopa',factor:1,component:'second'},
 {id:'stalevo',label:'Stalevo / Levodopa-Carbidopa-Entacapone',aliases:['stalevo'],cat:'levodopa',factor:1.33,component:'first'},
 {id:'madopar',label:'Madopar / Co-beneldopa',aliases:['madopar','medopar','co-beneldopa'],cat:'levodopa',factor:1,component:'first'},
 {id:'levodopa_ir',label:'Levodopa IR',aliases:['levodopa','l dopa'],cat:'levodopa',factor:1,component:'single'},
 {id:'pramipexole',label:'Pramipexole / Mirapex',aliases:['pramipexole','mirapex'],cat:'da',factor:100,component:'single'},
 {id:'ropinirole',label:'Ropinirole / Requip',aliases:['ropinirole','ropinrole','requip'],cat:'da',factor:20,component:'single'},
 {id:'apomorphine',label:'Apomorphine',aliases:['apomorphine'],cat:'da',factor:10,component:'single'},
 {id:'rasagiline',label:'Rasagiline / Azilect',aliases:['rasagiline','rasagline','azilect'],cat:'review',factor:null,component:'single'},
 {id:'selegiline',label:'Selegiline',aliases:['selegiline'],cat:'review',factor:null,component:'single'},
 {id:'amantadine',label:'Amantadine',aliases:['amantadine'],cat:'review',factor:null,component:'single'},
 {id:'entacapone',label:'Entacapone',aliases:['entacapone','comtan'],cat:'review',factor:null,component:'single'},
 {id:'tolcapone',label:'Tolcapone',aliases:['tolcapone','tasmar'],cat:'review',factor:null,component:'single'}
]);
function medM1Norm_(x){return String(x||'').normalize('NFKC').replace(/[，；]/g,m=>m==='，'?',':';').replace(/TRAN?DERMAL/ig,'TRANSDERMAL').replace(/\s+/g,' ').trim()}
function medM1Find_(line){const x=medM1Norm_(line).toLowerCase().replace(/[-_]+/g,' ');let best=null;MED_CATALOG_M1.forEach(d=>d.aliases.forEach(a=>{if(x.includes(a)&&(!best||a.length>best.alias.length))best={drug:d,alias:a}}));return best}
function medM1Words_(x){const m={one:1,two:2,three:3,four:4,five:5,six:6};return m[String(x||'').toLowerCase()]||Number(x)}
function medM1Times_(line){const x=medM1Norm_(line).toLowerCase();let m=x.match(/\b(one|two|three|four|five|six|\d+(?:\.\d+)?)\s+times?\s+(?:per\s+)?day\b/);if(m)return medM1Words_(m[1]);m=x.match(/\b(one|two|three|four|five|six|\d+(?:\.\d+)?)\s+time\s+(?:per\s+)?day\b/);if(m)return medM1Words_(m[1]);m=x.match(/\b(\d+(?:\.\d+)?)\s*(?:x|\/day)\b/);if(m)return Number(m[1]);m=x.match(/\b(?:once|daily|od|qd)\b/);if(m)return 1;m=x.match(/\b(?:twice|bd|bid)\b/);if(m)return 2;m=x.match(/\b(?:tds|tid)\b/);if(m)return 3;m=x.match(/\b(?:qds|qid)\b/);if(m)return 4;if(/patch(?:es)?\s+per\s+day/.test(x))return 1;m=x.match(/\b(one|two|three|four|five|six|\d+(?:\.\d+)?)\s+per\s+day\b/);return m?medM1Words_(m[1]):null}
function medM1Units_(line){const x=medM1Norm_(line).toLowerCase();let m=x.match(/\b(one|two|three|four|five|six|\d+(?:\.\d+)?)\s*(?:pills?|tablets?|tabs?|capsules?|caps?|patches?)\s+(?:per\s+time|each\s+time|per\s+dose|per\s+day)\b/);if(m)return medM1Words_(m[1]);m=x.match(/\b(one|two|three|four|five|six|\d+(?:\.\d+)?)\s+per\s+time\b/);if(m)return medM1Words_(m[1]);m=x.match(/\b(one|two|three|four|five|six|\d+(?:\.\d+)?)\s*(?:pills?|tablets?|tabs?|capsules?|caps?|patches?)\b/);return m?medM1Words_(m[1]):null}
function medM1Dose_(line,drug){const x=medM1Norm_(line);const ratios=[...x.matchAll(/(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)(?:\s*\/\s*(\d+(?:\.\d+)?))?/g)];if(ratios.length){const a=ratios[0];if(drug.component==='first')return Number(a[1]);if(drug.component==='second')return Number(a[2]);return Number(a[1])}const m=x.match(/(\d+(?:\.\d+)?)\s*mg\b/i);return m?Number(m[1]):null}
function parseMedicationM1_(raw){const lines=String(raw||'').replace(/\\n/g,'\n').split(/\n+|\s*;\s*/).map(medM1Norm_).map(x=>x.replace(/^\s*\d{1,2}\s*[.)、:]\s*/,'' )).filter(Boolean);const rows=[];let levodopa=0,da=0;lines.forEach((original,i)=>{const hit=medM1Find_(original);if(!hit){rows.push({line:i+1,original_text:original,drugId:'',name:original,strength:'',times:'',units:'',status:'unresolved',item_ledd:null,reason:'未識別藥物，已保留供人工選擇'});return}const d=hit.drug,dose=medM1Dose_(original,d),times=medM1Times_(original),units=medM1Units_(original);let status='ok',reason='',item=null;if(d.cat==='non_ledd'){status='not_applicable';reason='非PD LEDD藥物';item=0}else if(d.factor===null){status='review_required';reason='需按醫院權威結果核驗，不自動猜測係數'}else if(dose===null||times===null||units===null){status='unresolved';reason='規格、每次數量或每日次數未完整識別'}else{item=Math.round((dose*times*units*d.factor+Number.EPSILON)*100)/100;if(d.cat==='levodopa')levodopa+=item;if(d.cat==='da')da+=item}rows.push({line:i+1,original_text:original,drugId:d.id,name:d.label,strength:dose===null?'':String(dose)+' mg',times:times===null?'':String(times),units:units===null?'':String(units),status,item_ledd:item,reason,category:d.cat,factor:d.factor})});return {rows,levodopa:Math.round(levodopa*100)/100,da:Math.round(da*100)/100,total:Math.round((levodopa+da)*100)/100}}
function medM1CalcRows_(){let levodopa=0,da=0,unresolved=0;const rows=(ST.meds||[]).map(m=>{const d=MED_CATALOG_M1.find(x=>x.id===m.drugId);const dose=Number(String(m.strength||'').match(/\d+(?:\.\d+)?/)?.[0]),times=Number(m.times),units=Number(m.units);let item=null,status='ok';if(!d){status='unresolved';unresolved++}else if(d.cat==='non_ledd'){item=0;status='not_applicable'}else if(d.factor===null){status='review_required';unresolved++}else if(!Number.isFinite(dose)||!times||!units){status='unresolved';unresolved++}else{item=Math.round(dose*times*units*d.factor*100)/100;if(d.cat==='levodopa')levodopa+=item;if(d.cat==='da')da+=item}return Object.assign({},m,{item_ledd:item,status})});return {rows,levodopa:Math.round(levodopa*100)/100,da:Math.round(da*100)/100,total:Math.round((levodopa+da)*100)/100,complete:rows.length>0&&unresolved===0,unresolved}}
function medM1AddBlank_(){ST.meds.push({drugId:'',name:'',strength:'',times:'',units:'',original_text:'',verification_status:'pending'});saveDraft();renderByFlow()}
function medM1LoadParsed_(raw){const p=parseMedicationM1_(raw);ST.meds=p.rows.map(r=>({drugId:r.drugId,name:r.name,strength:r.strength,times:r.times,units:r.units,original_text:r.original_text,verification_status:r.status}));set('medication_raw_text',raw);setDerived('medication_machine_parse_json',JSON.stringify(p));saveDraft();renderByFlow()}
function renderMedicationRowsM1_(s){const rows=ST.meds||[];if(!rows.length){s.append(el('div','result warn','尚未加入藥物。貼上藥單後按「解析並建立藥物列」，或每次新增一款。'));return}rows.forEach((m,i)=>{const card=el('div','plain-block');card.append(el('h3','',`藥物 ${i+1}`));if(m.original_text)card.append(el('div','hint',`原文：${m.original_text}`));const sel=el('select');sel.append(new Option('選擇藥物',''));MED_CATALOG_M1.forEach(d=>sel.append(new Option(d.label,d.id)));sel.value=m.drugId||'';sel.onchange=()=>{m.drugId=sel.value;const d=MED_CATALOG_M1.find(x=>x.id===m.drugId);if(d)m.name=d.label;saveDraft();renderByFlow()};card.append(sel);const grid=el('div','form-grid');[['規格，例如 50 mg','strength'],['每次數量','units'],['每日次數','times']].forEach(([ph,k])=>{const inp=el('input');inp.placeholder=ph;inp.value=m[k]??'';inp.oninput=()=>{m[k]=inp.value;saveDraft()};inp.onchange=()=>renderByFlow();grid.append(inp)});card.append(grid);const calc=medM1CalcRows_().rows[i];card.append(el('div',calc.status==='ok'||calc.status==='not_applicable'?'result good':'result warn',calc.status==='ok'?`此藥LEDD：${calc.item_ledd} mg`:calc.status==='not_applicable'?'非PD LEDD藥物，保留記錄但不計入LEDD':calc.status==='review_required'?'待按醫院結果核驗，不自動猜測係數':'資料未完整或藥物未識別'));card.append(btn('刪除這款藥物',()=>{ST.meds.splice(i,1);saveDraft();renderByFlow()},'secondary'));s.append(card)})}
function renderMedicationWorkflowM1_(s){const raw=el('textarea');raw.placeholder='每款藥物一行';raw.value=val('medication_raw_text')||'';raw.oninput=()=>set('medication_raw_text',raw.value);s.append(el('h3','','Medication／LEDD'),raw);const actions=el('div','direct');actions.append(btn('解析並建立藥物列',()=>medM1LoadParsed_(raw.value),'primary'),btn('＋新增一款藥物',medM1AddBlank_,'secondary'));s.append(actions);renderMedicationRowsM1_(s);const c=medM1CalcRows_();setDerived('ledd_calculated_levodopa',c.levodopa);setDerived('ledd_calculated_da',c.da);setDerived('ledd_calculated_total',c.complete?c.total:null);setDerived('medication_manual_review_json',JSON.stringify(c));s.append(resultBox('系統解析摘要',[`Levodopa：${c.levodopa} mg`,`DA：${c.da} mg`,`Total：${c.total} mg`,c.complete?'所有納入LEDD的藥物資料完整。':`仍有 ${c.unresolved} 款需要核驗或補充；正式LEDD不可用。`],c.complete?'good':'warn'))}
renderMedicationRows=renderMedicationRowsM1_;
renderLeddPanel=function(s){renderMedicationWorkflowM1_(s)};
renderMedicationBF=function(s){renderMedicationWorkflowM1_(s)};
