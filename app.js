 ...
 * ========================================================================== */
const ADMIN_UI_BUILD='2.0-phase2';
...
async function adminPreviewIdentityV2_(data,previewHost,status){status.textContent='正在檢查身份碰撞及受影響事件……';const p=await adminApiV1_('identity_preview',data);previewHost.innerHTML='';previewHost.append(resultBox('身份修改影響預覽',[`原P_ID：${p.old_p_id||'未分配'} → 新P_ID：${p.new_p_id||'未分配'}`,`S_ID：${p.s_id||'—'}`,`受影響正式事件：${p.affected_event_count}筆`,`將建立Correction：${p.affected_event_count}筆`,`P_ID Registry匹配：${p.registry_rows_matching_pid}行 ｜ S_ID Registry匹配：${p.registry_rows_matching_sid}行`,`阻擋：${p.blockers.join('；')||'沒有'}`,`警告：${p.warnings.join('；')||'沒有'}`],p.blockers.length?'bad':p.warnings.length?'warn':'good'));status.className=p.blockers.length?'result warn':'result good';status.textContent=p.blockers.length?'身份修改被阻擋。':'身份預覽完成。';return p}
