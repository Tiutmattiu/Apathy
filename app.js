...
async function renderUBSNAssistant(){
 const m=appShell();m.append(toolbar('MRI Booking Assistant'));const s=el('section','summary ubsn-assistant');const status=el('div','result','正在連接本機UBSN helper……'),actions=el('div','ubsn-actions');
 s.append(el('h2','section-title','Human MRI 時段助手'),status,btn('立即檢查UBSN',async()=>{const now=new Date(),end=new Date(now.getTime()+7*86400000);status.textContent='正在檢查……';try{await ubsnFetch('/check-now',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({start:now.toISOString(),end:end.toISOString()})});await refresh()}catch(e){offline(e)}},'primary'),el('h3','','目前可用機會'),actions);m.append(s);
 const offline=()=>{status.className='result warn';status.textContent='UBSN helper is not running on this device.';actions.innerHTML=''};
 const refresh=async()=>{try{await ubsnFetch('/health');const rows=await ubsnFetch('/actions');status.className='result good';status.textContent='● 本機UBSN helper已連接';actions.innerHTML='';if(!rows.length)actions.append(el('p','hint','目前沒有符合參與者條件的新時段。'));rows.filter(x=>['NEW','READY'].includes(x.status)).forEach(x=>{const card=el('div','result');const start=new Date(x.slot.start),end=new Date(x.slot.end);card.append(el('strong','',`${start.toLocaleDateString('zh-HK')}　${start.toLocaleTimeString('zh-HK',{hour:'2-digit',minute:'2-digit'})}–${end.toLocaleTimeString('zh-HK',{hour:'2-digit',minute:'2-digit'})}`),el('p','',`Matches：${x.matching_pids.join('、')}　建議：${x.recommended_pid}`),btn('Prepare booking',async()=>{try{await ubsnFetch(`/actions/${x.action_id}/prepare`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});card.append(el('p','hint','已在本機瀏覽器開啟；CAPTCHA及最終確認由工作人員完成。'))}catch(e){status.className='result warn';status.textContent=e.message}},'primary'));actions.append(card)})}catch(e){offline(e)}};
 await refresh()
}
...
function guidedRenderStopped_(){
  ...
  s.append(
    ...
    el(
      'div',
      'result warn',
      '根據本研究的篩選規則，研究人員需要先跟您確認一些資料。請不要繼續填寫後面的問卷。'
    ),
    el(
      'p',
      '',
      '請等待研究人員聯絡您。如研究人員曾提供聯絡方式，也可以直接聯絡研究人員。'
    )
  );

  if(guidedDryRun_()){
    s.append(el('p','hint','測試模式：這次沒有把資料送到後端。'));
  }
