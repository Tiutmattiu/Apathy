...
function renderChoice(pg,a){const g=el('div','direct');pg.options.forEach(o=>{const b=btn(o[1],()=>{if(b.dataset.advancing==='1')return;set(pg.key,o[0]);if(pg.key==='education_level'&&o[2]!==null)set('education_years',o[2]);if(pg.key==='pd_status_self_report')set('pd_hc_status',Number(o[0])===1?'PD':'HC');b.classList.add('selected');if(ST.flow==='stage2'){b.dataset.advancing='1';qa('button',g).forEach(x=>x.disabled=true);setTimeout(()=>{if(playerPages()[ST.step]===pg&&pageComplete(pg))autoNext()},320)}else setTimeout(autoNext,120)},'choice'+(sameValue(val(pg.key),o[0])?' selected':''));g.append(b)});a.append(g);if(pg.key==='education_level'&&val(pg.key)==='other'){const f=fieldText('實際教育年數','education_years','請輸入年數','number');a.append(f)}}
function renderInput(pg,a){const i=el('input','text');i.placeholder=pg.placeholder||'';i.value=val(pg.key)??'';if(ST.flow==='stage2'){i.enterKeyHint='done';if(pg.key==='contact_phone')i.inputMode='tel'}i.oninput=()=>set(pg.key,i.value);const commit=()=>{if(ST.flow!=='stage2'||i.dataset.advanced==='1')return;const current=playerPages()[ST.step];if(current!==pg)return;const ok=pg.key==='contact_phone'?/^\d{8,15}$/.test(String(i.value||'').replace(/\D/g,'')):String(i.value||'').trim().length>0;if(!ok)return;i.dataset.advanced='1';autoNext()};i.onkeydown=e=>{if(e.key==='Enter'&&i.value.trim()){e.preventDefault();if(ST.flow==='stage2')commit();else autoNext()}};if(ST.flow==='stage2'){i.onchange=commit;i.onblur=()=>setTimeout(commit,0)}a.append(i)}
...
function renderScale(pg,a){
  ...
  opts.forEach((o,n)=>{const keyText=opts.length===5?String(o.value):String(n+1),b=btn('',()=>{if(b.dataset.advancing==='1')return;set(pg.key,o.value);b.classList.add('selected');if(ST.flow==='stage2'){b.dataset.advancing='1';qa('button',g).forEach(x=>x.disabled=true);setTimeout(()=>{if(playerPages()[ST.step]===pg&&pageComplete(pg))autoNext()},320)}else autoNext()},(opts.length===5?'':'choice')+(val(pg.key)===o.value?' selected':''));b.append(el('strong','',keyText),document.createTextNode(' '+String(o.label).replace(/^\d+\s*/,'')));g.append(b)});a.append(g)
}
...
function renderExamples(pg,a){
  ...
  const t=el('textarea');t.placeholder=ST.flow==='stage2'?'請填寫一項平日活動':(domain?.examplePlaceholder||'請在此輸入');t.value=val(pg.key)||'';t.oninput=()=>set(pg.key,t.value);t.onkeydown=e=>{if((e.ctrlKey||e.metaKey||e.altKey)&&e.key==='Enter'){e.preventDefault();if(splitExamples(t.value).length>=min)autoNext();else{ST.error=`請至少輸入${min}項。`;player()}}};a.append(t,el('p','hint',ST.flow==='stage2'?'填寫一項即可，完成後按下方「繼續」。':'可按「完成並繼續」；電腦亦可按 Ctrl／⌘／Alt + Enter。一般 Enter 只會換行。'))
}
...
function renderDOB(a){const row=el('div','date-row'),names=[['dob_d','DD',2],['dob_m','MM',2],['dob_y','YYYY',4]],ageBox=el('div');function commit(go,source){const d=+val('dob_d'),m=+val('dob_m'),y=+val('dob_y'),dt=new Date(y,m-1,d);if(!d||!m||!y||dt.getFullYear()!==y||dt.getMonth()!==m-1||dt.getDate()!==d||dt>new Date())return false;set('date_of_birth',`${y}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`);const age=calcAge();set('age_years',age);ageBox.className='result good';ageBox.textContent=`年齡：${age}歲（自動計算）`;if(go&&source?.dataset.advanced!=='1'){if(source)source.dataset.advanced='1';autoNext()}return true}names.forEach((x,n)=>{const i=el('input','digits');i.inputMode='numeric';i.maxLength=x[2];i.placeholder=x[1];i.value=val(x[0])||'';if(ST.flow==='stage2'&&n===2)i.enterKeyHint='done';i.oninput=()=>{i.value=i.value.replace(/\D/g,'').slice(0,x[2]);set(x[0],i.value);if(i.value.length===x[2]&&n<2)qa('input',row)[n+1].focus();if(n===2&&i.value.length===4)commit(false,i)};i.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();commit(true,i)}else if(e.key==='Backspace'&&!i.value&&n>0)qa('input',row)[n-1].focus()};if(ST.flow==='stage2'&&n===2){const done=()=>setTimeout(()=>commit(true,i),0);i.onchange=done;i.onblur=done}row.append(i)});a.append(row,ageBox);if(calcAge()!==null){ageBox.className='result good';ageBox.textContent=`年齡：${calcAge()}歲（自動計算）`}}
