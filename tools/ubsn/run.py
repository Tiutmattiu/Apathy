...
def main():
    ...
    p.add_argument("command",choices=("capture","check","serve"));p.add_argument("--config",default="config.local.json");p.add_argument("--start",required=False);p.add_argument("--end",required=False)
    args=p.parse_args();config=json.loads(Path(args.config).read_text(encoding="utf-8"));logging.basicConfig(level=logging.INFO,format="%(asctime)s %(levelname)s %(message)s")
