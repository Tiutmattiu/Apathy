...
def serve(watcher, host="127.0.0.1", port=8765, monitor=False):
    ...
    class Handler(BaseHTTPRequestHandler):
        def allowed_origin(self):
            ...
            return origin is None or origin in watcher.config.get(
                "allowed_origins", ["null", "http://127.0.0.1:8000", "http://localhost:8000"]
            )
        ...
        def do_GET(self):
            ...
            if path == "/health":
                self.send_json(200, {"connected": True})
            elif path == "/status":
                self.send_json(200, watcher.status())
            elif path == "/actions":
